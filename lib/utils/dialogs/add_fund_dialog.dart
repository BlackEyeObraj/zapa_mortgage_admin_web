...
class FundDialog{
  ...
  editFundDialog(String borrowerID,int index, String assetType,String accountNumber, String currentBalance, String bankName, bool userVerifiedFund,String status, String addedBy, String verifyStatusOld, String borrowerName, String borrowerPhoneNumber){
    ...
    Get.put(FundsViewController()).setVerifyCheck('verified', verifyStatusOld == 'Verified'?true:false);
    Get.put(FundsViewController()).setVerifyCheck('notVerified', verifyStatusOld == 'Verified But Excluded'?true:false);
    Get.put(FundsViewController()).selectedAssetTypeFund.value = assetType;
    Get.put(FundsViewController()).setSelectedAssetTypeEnableOrDisable(userVerifiedFund);
    Get.put(FundsViewController()).selectedAddedBy.value = addedBy;
          ...
          child: Column(
            ...
            children: [
              ...
              Text('Asset Type',style: TextStyle(fontWeight: FontWeight.bold,color: AppColors.primaryColor,
                  fontSize: 12),),
              DropdownButtonFormField2<String>(
                value: Get.put(FundsViewController()).selectedAssetTypeFund.value,
                isExpanded: false,
                decoration: InputDecoration(
                  // Add Horizontal padding using menuItemStyleData.padding so it matches
                  // the menu padding when button's width is not specified.
                    ...
                    onPressed: () async{
                      if(Get.put(FundsViewController()).selectedAssetTypeFund.value == ''){
                        ...
                        SnackBarApp().errorSnack('Info Incomplete', 'Please enter current balance');
                           ...
                           FirestoreService().updateFundValues(
                               Get.put(FundsViewController()).selectedAddedBy.value,
                               borrowerID,
                               index,
                               bankNameController.text,
                               accountNumberController.text,
    ...
    //                     SnackBarApp().errorSnack('Info Incomplete', 'Please enter current balance');
    //                   }else{
    //                     FirestoreService().updateFundValues(borrowerID,index, bankNameController.text,
    //                         accountNumberController.text, amountController.text, Get.put(FundsViewController()).selectedAssetTypeFund.value,Get.put(FundsViewController()).selectedAssetTypeEnable,statusCurrent);
    //                     Get.back();
    //                   }
    //                 },
    //                 style: ButtonStyle(
